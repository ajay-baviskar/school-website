    ...
    <div class="row">
        <div class="col-md-12">
            <!-- <div class="card mb-5">
                ...
                <div class="card-body"> -->
                    @role(['Zone User','Plant Coordinator','HOD'])
...
<script>
    ...
    $(document).ready(function(){
        ...
        .then(response => {
            implement_data = response.data.all_data;
            assigned_data = response.data.assigned_data;
            hold_data = response.data.closed_data;
            in_progress_data = response.data.open_data;
        ...
        function mapData(array) {
            return array.map(function(item) {
                if (item.get_feedback_data.feedback_text1) {
                    return {
                        // 'Custom ID': item.id,
                        ...
                        'Completion Date': item.date_of_completion,
                        'Description': item.get_suggestion_data.desc,
                        'Priority': item.get_suggestion_data.priority,
                        'Score': item.feedback_score,
                        'Coardinator Feedback': item.get_feedback_data.feedback_text1,
                        'HOD Feedback': item.get_feedback_data.feedback_text2,
                        'Closing Remark': item.get_feedback_data.closing_remark,
        ...
        function exportToExcel() {
            ...
            XLSX.utils.book_append_sheet(wb, sheet1, 'All');
            XLSX.utils.book_append_sheet(wb, sheet2, 'Open');
            XLSX.utils.book_append_sheet(wb, sheet3, 'Closed');
        ...
        $('#filter-form').on('submit', function(event) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $("#addModal").modal('hide');
                    implement_data = response.all_data;
                    assigned_data = response.assigned_data;
                    hold_data = response.closed_data;
                    in_progress_data = response.open_data;
        ...
        var totalPages2 = Math.ceil(in_progress_data.length / itemsPerPage2);
        var currentPage3 = 1;
        var itemsPerPage3 = 10;
        var totalPages3 = Math.ceil(hold_data.length / itemsPerPage3);

        var currentPage4 = 1;
        var itemsPerPage4 = 10;
        var totalPages4 = Math.ceil(pointed_data.length / itemsPerPage4);

        var currentPage5 = 1;
        var itemsPerPage5 = 10;
        var totalPages5 = Math.ceil(created_data.length / itemsPerPage5);

        var currentPage6 = 1;
        var itemsPerPage6 = 10;
        ...
        function renderProducts(page,implement_data,ids) {
            ...
            paginatedProducts.forEach(function(product) {
                ...
                if (product.status != 'Closed') {
                    SendEmail = `&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-danger" href="suggestion/${product.id}?send_email=1"  class="" style="float: right;margin-left: 12px;margin-bottom: 12px;" >Send Email</a><br/>`;
                }
                if(product.get_suggestion_data){
                    var productCard = `
                        <div class="task-card card">
                        <div class="task-details">
                            <div>
                            <div class="task-title">#${product.id} - ${product.get_suggestion_data.title}</div>
                            <div class="task-meta">
                                <span class="badge badge-pill badge-soft-primary"><i class="fas fa-info"></i>&nbsp;${product.get_type.name}</span>
        ...
        $('.search-items').keyup(function(){
            ...
            if ($(this).attr('id') == 'implement_datas') {
                var filteredProducts = implement_data.filter(function(product) {
                    if(product.get_suggestion_data){
                        let name = (product.get_created_by.name).toLowerCase().includes(searchText);
                        let title = (product.get_suggestion_data.title).toLowerCase().includes(searchText);
                        return (name || title);
                    }
                });
